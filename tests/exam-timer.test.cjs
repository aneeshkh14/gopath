...
test('Next, Review and the palette cannot enter a future timed section',async t=>{
 const p=await page();t.after(p.close);assert.equal(p.$('#gep-next-btn').text(),'Save Answer');p.$('#gep-next-btn').trigger('click');await ok(p.fetches[0]);p.$('#gep-review-btn').trigger('click');await ok(p.fetches[1]);p.$('.gep-palette-btn[data-id="2"]').trigger('click');
 assert.equal(p.$('.gep-question-block[data-id="1"]')[0].style.display,'block');assert.equal(p.$('.gep-section-tab.active').data('cat-id'),'sec_0');assert.deepEqual(p.errors,[]);
});
test('Previous and the palette cannot reopen an expired section after resume',async t=>{
 const p=await page({elapsed:90,remaining:510});t.after(p.close);assert.equal(p.$('#gep-prev-btn').prop('disabled'),true);p.$('#gep-prev-btn').trigger('click');p.$('.gep-palette-btn[data-id="1"]').trigger('click');
 assert.equal(p.$('.gep-question-block[data-id="2"]')[0].style.display,'block');assert.equal(p.fetches.length,0);
});
test('a delayed heartbeat cannot rewind the clock across a section boundary',async t=>{
 const p=await page({elapsed:59,remaining:541});t.after(p.close);const beat=p.heartbeat();await p.advance(2);const save=p.fetches.find(x=>x.data.action==='gep_save_answer');if(save)await ok(save);
 p.fetches[0].resolve({success:true,data:{remaining_seconds:541}});await beat;assert.equal(p.$('#gep-timer').text(),'00:00:59');assert.equal(p.$('.gep-section-tab.active').data('cat-id'),'sec_1');
});
