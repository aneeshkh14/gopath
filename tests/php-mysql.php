...
seed();$free=(new GEP_Payment())->create_order(3,'test');expect(!is_wp_error($free) && $free['status']==='free');expect($wpdb->get_var('SELECT status FROM wp_gep_orders WHERE item_id=3')==='success');expect((int)$wpdb->get_var('SELECT COUNT(*) FROM wp_gep_user_test_access WHERE test_id=3')===1);echo "PASS MySQL free enrollment creates completed order and access\n";
