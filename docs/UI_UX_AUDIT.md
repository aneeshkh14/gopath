...
## Exam reference buttons — v2.0.8

The customer reported that tapping Instructions returned to questions from the mobile question palette. Source inspection found the palette capture handler closed Instructions before its own click handler, referenced a nonexistent Question Paper button ID, and left reference popups below the full-size palette's stacking level. The two real buttons now close the palette within their own handlers and open only their matching popup. Reference popups sit above the palette and temporarily make the underlying exam grid inert; closing restores grid interaction and focus. Calculator behavior remains scoped to its existing dialog.

The production button IDs and both reference dialogs are now present in the exam fixture. Six new DOM cases cover Instructions and Question Paper at 390, 820 and 1366 pixels, each exercising close button, Escape and backdrop dismissal while preserving the current question and typed answer. Eight existing stylesheet cases now assert that the popup layer is above the palette. The targeted exam suite passes all 30 cases. With the full CI suite the expected total is **115 frontend + 38 isolated PHP + 5 MySQL + 146 WordPress = 304 cases**. CI results are linked in the fix PR.

The supplied screenshot was reviewed. These checks cover DOM events and stylesheet cascade; they do not claim a new live mobile-browser reproduction. The fix needs deployment to WordPress before the customer can verify the updated behavior.

## Palette follow, theme readability and posters — v2.0.9

The supplied video shows navigation progressing beyond question 80 while the laptop palette remains on questions 21–70. Question loading now keeps the active numbered button inside the scrollable palette viewport using the smallest vertical adjustment. It moves only that grid, follows backwards as well as forwards, updates `aria-current`, and reveals the current number when the overview opens. The desktop edge handle now opens the same full-size palette as the header button. Compact laptop grid/action gutters are reduced from 25px to 10px; phone spacing stays under the existing rules.

The video also shows a light action panel inside the dark exam. Exam action surfaces/text now use the selected theme consistently. Pasted question, option, passage and reference text colours follow the paper surface in both themes, including nested inline text colours; images and diagram fills remain untouched. Status numbers use darker green/red/purple fills with readable white text. Reference headings, labels, counters, and controls are covered by the same colour pass.

The customer's screenshot notes that swipeable posters disappeared from the dashboard top. Existing configured promotion images, titles, descriptions and links are restored above the study area in a horizontal scroll-snap strip with previous/next controls. Native swiping and keyboard scrolling work without automatic rotation; arrow scrolling respects reduced-motion preference. The saved-test action remains below the posters.

New verification: six DOM cases cover a 100-question Next/Previous sequence at phone/tablet/laptop widths, no unnecessary movement for an already visible number, desktop expansion without page movement, and poster controls. Two stylesheet cases check foreground/background contrast of exam text and status/action controls in both themes, including nested inline text colours. Theme variables are resolved to their declared values for the JSDOM cascade check; this is not a claim of full browser layout. One WordPress case verifies configured posters precede the study area without losing resume. Full expected CI suite: **123 frontend + 38 isolated PHP + 5 MySQL + 147 WordPress = 313 cases**.

The two screenshots and sampled video frames were inspected. Actual live phone/tablet/laptop rendering is not certified by these automated checks; no new authenticated live test was performed for this patch. Version 2.0.9 must be deployed before the customer can verify it on the site. This targets the identified exam/readability surfaces, not a claim that every possible uploaded image or unknown page has been audited.

## Direct support contact — v2.0.10
